...
for helper in "${AUR_HELPERS[@]}"; do
    if command -v "$helper" &>/dev/null; then
        # Use 'paru' or 'yay' for version check if the helper is the main command or a symlink
        local_cmd=$([[ "$helper" == "paru-bin" ]] || [[ "$helper" == "yay-bin" ]] ? "${helper%-bin}" : "$helper")
        
        # Check if the command is available for version check
...
BUILD_DIR=$(mktemp -d)
trap 'rm -rf "$BUILD_DIR"' EXIT # Cleanup trap
